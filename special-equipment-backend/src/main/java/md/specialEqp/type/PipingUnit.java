...
public class PipingUnit {
    ...
    private Float len;  //LENGTH；管道长度m
}

//不可改参数：EQP_CODE; LENGTH;管道起点START_PLACE;管道止点END_PLACE;管道单元登记编号EQP_UNT_REGCOD;
//附带不可改属性： 使用状态USE_STA； 年检报告编号YEAR_ISP_REPORT_COD； 下次年检日期YEAR_NEXT_ISP_DATE；年检结论YEAR_ISP_CONCLU；年检日期YEAR_ISP_DATE；
//   定检检验报告编号ISP_REPORT_COD；定检下检日期NEXT_ISP_DATE；定检结论ISP_CONCLU；定检日期ISP_DATE；
//   监检日期INCP_ISP_DATE；监检报告编号INCP_ISP_REPORT_COD；监检报告下检日期INCP_NEXT_ISP_DATE；监检结论INCP_ISP_CONCLU；
